...
MapLoader::MapLoader(ros::NodeHandle& nh)
{
    ...
    nh.param<std::string>("map_point", map_topic, "point_map");

    init_tf_params(nh);             // 第一个函数：初始化参数
    ...
    auto pc_msg = CreatePcd();         // 第二个函数：创建pcd

    auto out_msg = TransformMap(pc_msg);    // 第三个函数

    if ( out_msg.width != 0 )
    {
        out_msg.header.frame_id = "map";
        pc_map_pub_.publish(out_msg);   // 发布
    }
}
...
sensor_msgs::PointCloud2 MapLoader::TransformMap(sensor_msgs::PointCloud2& in)
{
    ...
    Eigen::AngleAxisf rot_x_m2w(tf_roll_, Eigen::Vector3f::UnitX());    // rot: rotation，欧拉角的表示方式
    ...
    SaveMap(transformed_pc_ptr);        // 将转换后的点云保存一下
...
sensor_msgs::PointCloud2 MapLoader::CreatePcd()
{
    ...
    for (const std::string& path : file_list_ )
    {
        if (0 == pcd.width)
        {
            if (-1 == pcl::io::loadPCDFile(path.c_str(), pcd))
            {
        ...
        {
            if (-1 == pcl::io::loadPCDFile(path.c_str(), part))
            {
                std::cerr << "load failed " << path << std::endl;
            }
...
{
    ros::init(argc, argv, "map_loader");        // 指定节点名字
